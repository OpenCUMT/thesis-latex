...
\begin{itemize}
    \item \texttt{output}：设置输出格式，可选 \texttt{electronic}（电子版）、\texttt{print}（打印版）或\texttt{blindreview}（盲审版）\footnote{盲审版仅保留封面页和论文主体部分}或\texttt{inspection}（抽检版）\footnote{抽检版隐藏学校、人名、校徽等敏感信息，并隐去致谢部分}。
...
\begin{equation}
  \mathcal{L}_F(\theta, \psi, \xi, \epsilon, \eta) = \mathcal{KL}(\mathcal{N}(\mu_x, \sigma_x^2) || \mathcal{N}(\mu_y, \sigma_y^2))
\end{equation}
\begin{equation}
  \hat{\mathbf{y}}=\mathrm{argmax}_y\ \mathcal{D}_\vartheta(\mathcal{F}_\theta(\mathbf{X}_\sigma)).
\end{equation}

\subsection{化学公式}
模板中使用 \textsf{mhchem} 宏包实现化学公式。使用\verb|\ce{}|输入。
...
\subsection{表}
模板引入了\textsf{tabularx}宏包。同时自定义列类型\texttt{C}，用于创建固定宽度且内容水平居中的表格列，即 \verb|C{<type>}{<width>}|。其中，\texttt{\#1} 指定列的基本类型（如 \texttt{p}、\texttt{m} 或 \texttt{b}），\texttt{\#2} 是列宽。

\subsection{算法}
模板中使用 \textsf{algorithm2e} 宏包实现算法环境。具体用法参考官方文档。
